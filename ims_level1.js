document.addEventListener('DOMContentLoaded', loadLevel1Items);

function loadLevel1Items() {
...
function displayLevel1Items(items) {
    const tbody = document.getElementById('level1-body');
    tbody.innerHTML = '';

    // Only show Level 1 items
    const level1Items = items.filter(item => item.level === 'Level_1');

    if (!level1Items.length) {
        const row = document.createElement('tr');
        row.innerHTML = '<td colspan="5" style="text-align: center;">No Level 1 items found</td>';
...
document.getElementById('addBtn').addEventListener('click', () => {
    ...
    if (allLevel1Items.some(item => item.name.toLowerCase() === name.toLowerCase())) {
